...
class MiniAppServer:
    ...
    async def start(self) -> bool:
        ...
        app.router.add_get("/mini-app", self._redirect_to_index)
        app.router.add_post("/mini-app/api/auth", self._authenticate)
        app.router.add_get("/mini-app/api/status", self._status)
        app.router.add_static("/mini-app/static/", _STATIC_DIR, show_index=False)
    ...
    async def _authenticate(self, request: web.Request) -> web.Response:
        ...
        except (MiniAppAuthError, json.JSONDecodeError, AttributeError, TypeError):
            return web.json_response(
