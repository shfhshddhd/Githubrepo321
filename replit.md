...
- Users can create and remove permanent group-to-target mappings.
- Target messages are forwarded to Saved Messages and replies are bridged back into the source group.

## User preferences

- The user wants the Telegram bot restored and running.
- Gemini keys added with `/addkey` are stored in Replit Database when available (with a local JSON fallback) and take priority over the `GEMINI_API_KEY` environment fallback.
...
- Mappings are keyed by hosted user, group chat ID, and target user ID; adding an existing pair updates it.
